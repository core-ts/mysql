...
function buildToSave(obj, table, attrs, ver, buildParam, i) {
  ...
  if (pks.length === 0) {
    ...
    if (colSet.length === 0) {
      var q = "insert ignore into " + table + "(" + cols.join(',') + ")values(" + values.join(',') + ")";
      return { query: q, args: args };
    }
    else {
      var q = "insert into " + table + "(" + cols.join(',') + ")values(" + values.join(',') + ") on duplicate key update " + colSet.join(',');
      return { query: q, args: args };
    }
  }
}
